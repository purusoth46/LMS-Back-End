...
const cookieOptions = {
  expires: new Date(
    Date.now() + process.env.JWT_COOKIE_EXPIRY * 24 * 60 * 60 * 1000), // 30 days
  httpOnly: true,
  secure: true,
  sameSite: "none",
...
*/
  ...
  try {
    ...
    if (!user) {
      return next(
        new AppError("User is null , unable to fetch forgetPasswordToken", 500)
      );
    }
    ...
    return next(
      new AppError(error.message || "Unable to Update the password", 400)
    );
  }
};
...
const ChangePassword = async (req, res, next) => {
  ...
  if (!oldPassword || !newPassword) {
    return next(
      new AppError("Enter Both password , oldPassword and NewPassword", 400)
    );
  }
...
const updateUser = async (req, res, next) => {
  ...
  if (req.file) {
    try {
      ...
      return next(
        new AppError(error.message || "unable to update details ", 500)
      );
    }
  }
...
const makeUserAdmin = async (req, res, next) => {
  ...
  try {
    ...
    return next(
      new AppError(error.message || "error in changing the role ", 400)
    );
  }
};
